...
BEGIN
    ...
    BEGIN 
        INSERT INTO [dbo].[StudentLogin]
               ...
               ,[IsLogin],[LastLoginIP],[LastLoginDate],[LastLoginDevice],[LastLoginLocation],[IsActive],[IsDelete],[ModifyDate],[ModifyBy],[AddedDate]
               ,[AddedBy])
         VALUES
               (@UserName,@NormalizedUserName,@Email,@NormalizedEmail,@EmailConfirmed,@PasswordHash,@PhoneNumber,@PhoneNumberConfirmed
               ,@TwoFactorEnabled,@LockoutEndDate,@LockoutEnabled,@AccessFailedCount,@SecurityStamp,@FirstName,@LastName,@Password
               ,@IsLogin,@LastLoginIP,@LastLoginDate,@LastLoginDevice,@LastLoginLocation,@IsActive,0,GETDATE(),@AddedBy,GETDATE()
               ,@AddedBy)
