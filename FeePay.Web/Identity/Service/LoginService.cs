...
namespace FeePay.Infrastructure.Identity.Service
{
    public class LoginService : ILoginService
    {
        ...
        public LoginService(
            ...
            SignInManager<SchoolAdminUser> SignInManagerSchool,
            SignInManager<StudentLogin> SignInManagerStudent,
            SignInManager<SuperAdminUser> SignInManagerSuperAdmin,
            UserManager<SchoolAdminUser> UserManagerSchool,
            UserManager<StudentLogin> UserManagerStudent,
        ...
        public async Task<Response<bool>> AuthenticateSchoolUserAsync(SchoolLoginViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                var session = await _unitOfWork.Session.FetchActiveAcadmicSession(model.SchoolUniqueId);
                if (session != null)
                {
        ...
        public async Task<List<string>> GetUserAccessRights(string userName, string areaName)
        {
            ...
            if (areaName.ToLower() == "superadmin")
            {
            ...
            else if (areaName.ToLower() == "school")
            {
        ...
        public async Task<List<string>> GetUserAccessRoles(string userName, string areaName)
        {
            List<string> roles_Access = new List<string>();
            if (areaName.ToLower() == "superadmin")
            {
            ...
            else if (areaName.ToLower() == "school")
            {
                var SchoolId = _appContextAccessor.ClaimSchoolUniqueId();
                var user = await _unitOfWork.SchoolAdminUser.FindActiveByUserNameAsync(userName, SchoolId);
                roles_Access = user?.Roles?.Select(r => r.Name).ToList();
            }
            else
            {
                throw new Exception("not supported area.");
        ...
        public async Task<Dictionary<string, string>> GetCurrentStudentClassSection()
        {
            ...
            try
            {
                ...
                if (studentProfile != null)
                {
                    ...
                    {
                        var _class = await _unitOfWork.ClassRepo.FindByIdAsync(id: studentProfile.ClassId, dbId: SchoolUniqueId, isActive: true);
                        dic.Add("class", _class?.Name);
                    }
                    if (studentProfile.SectionId > 0)
                    {
                        var section = await _unitOfWork.SectionRepo.FindByIdAsync(id: studentProfile.SectionId, dbId: SchoolUniqueId, isActive: true);
        ...
        public async Task<SessionViewModel> GetCurrentAcademicSession()
        {
            var SchoolId = _appContextAccessor.ClaimSchoolUniqueId();
            if (_appContextAccessor.SiteSession_AcademicSession == null)
            {
                // If session is expire logout and return null 
                await SchoolAdminLogout();
                return null;
            }
            return _appContextAccessor.SiteSession_AcademicSession;
        }
